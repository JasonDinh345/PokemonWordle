...
export default function SplitBanner({ children, className }: SplitBannerProps) {
    ...
    return (
        <SplitBannerContext.Provider value={true}>
            <div className={`w-full h-32 flex flex-col items-center justify-center bg-stone-200 lg:h-84 ${className ? className : ""}`}>
                <div className={`flex flex-row items-center justify-center gap-8 lg:gap-16 h-full`}>
                    {children[0]}
                    {children[1]}
                    {children[2]}
                </div>
                {width<640 && <p className="pb-2">Touch to see details!</p>}
            </div>
